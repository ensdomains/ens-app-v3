...
test('should maintain state and when returning from transaction modal when adding/updating records',async ({page, login, makeName, makePageObject}) => {
  const name = await makeName({
    label: 'profile',
    type: 'legacy',
    records: {
  ...
  await advancedEditor.recordInput('text', 'another text').then((input) => input.fill('another record'))
  ...
  await advancedEditor.recordInput('address', 'BNB').then((input) => input.fill('bnb1g5p04snezgpky203fq6da9qyjsy2k9kzr5yuhl'))
  await advancedEditor.recordInput('address', 'ETH').then((input) => input.fill('0x70997970C51812dc3A010C7d01b50e0d17dc79C8'.toLowerCase())) // lowercase to test normalization
  
  // Update other records
  await advancedEditor.recordInput('contentHash').then((input) => input.fill('bzz://e40101fa011b20d1de9994b4d039f6548d191eb26786769f580809256b4685ef316805265ea162'))
  ...
  await expect(await advancedEditor.recordInput('text', 'another text')).toHaveValue('another record')
  await expect(await advancedEditor.recordInput('address', 'BNB')).toHaveValue('bnb1g5p04snezgpky203fq6da9qyjsy2k9kzr5yuhl')
  await expect(await advancedEditor.recordInput('address', 'ETH')).toHaveValue('0x70997970C51812dc3A010C7d01b50e0d17dc79C8')
  await expect(await advancedEditor.recordInput('contentHash')).toHaveValue('bzz://e40101fa011b20d1de9994b4d039f6548d191eb26786769f580809256b4685ef316805265ea162')
  ...
  await expect(recordsPage.getRecordValue('address', 'eth')).toHaveText('0x70997970C51812dc3A010C7d01b50e0d17dc79C8')
  await page.pause()
  await expect(recordsPage.getRecordValue('contentHash')).toHaveText('bzz://e40101fa011b20d1de9994b4d039f6548d191eb26786769f580809256b4685ef316805265ea162'.toLowerCase())
