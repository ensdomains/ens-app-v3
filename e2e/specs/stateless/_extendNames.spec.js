...
describe('Extend Names', () => {
  ...
  it('should extend a single name', () => {
    ...
    cy.get('@timestamp').then(timestamp => {
      ...
      cy.log('new timestamp: ' + newTimestamp)
      cy.findByTestId('expiry-label').invoke('attr', 'data-timestamp').then(t => {
        cy.log('new label: ' + t)
      })
  ...
  it('should not show check button on address page if user is not logged in', () => {
    cy.visit('/my/settings')
    cy.get('button > div').contains('Disconnect').parent().click()
    cy.visit('/address/0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266')
    cy.findByTestId('name-item-other-registrant.eth').should('be.visible')
    cy.findByTestId('check-button').should('not.exist')
  })

  it('should show disabled extend button on details page if user is not logged in', () => {
    cy.visit('/profile/other-registrant.eth/details')
    cy.findByTestId('extend-button').should('be.disabled')
  })
