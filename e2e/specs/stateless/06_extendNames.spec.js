...
describe('Extend Names', () => {
  ...
  it('should extend a single name', () => {
    ...
    cy.findByTestId('invoice-item-1-amount').should('contain.text', '$0.20')
    cy.findByTestId('invoice-total').should('contain.text', '$10.20')
    cy.findByTestId('plus-minus-control-minus').click()
    cy.findByTestId('invoice-item-0-amount').should('contain.text', '$5.00')
    cy.findByTestId('invoice-item-1-amount').should('contain.text', '$0.20')
    cy.findByTestId('invoice-total').should('contain.text', '$5.20')
  ...
  it('should be able to extend multiple names on the names page', () => {
    ...
    cy.wait(10000)
    cy.findByTestId('extend-names-confirm').should('not.be.disabled').click()
