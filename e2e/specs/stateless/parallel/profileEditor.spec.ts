...
const getDefaultRecords = (address: Address | undefined) =>
  ({
    texts: [
      {
        key: 'description',
...
const makeRecords = async (
  { accounts, user = 'user' }: { accounts: Accounts; user?: User },
  overwrites: RecordOptions = {},
): Promise<RecordOptions> => {
  const defaultRecords = getDefaultRecords(accounts.getAddress(user) as Address)
  const texts = [...defaultRecords.texts!, ...(overwrites.texts || [])].reduce<
    RecordOptions['texts']
  >((acc, curr) => {
  ...
  const coins = [...defaultRecords.coins!, ...(overwrites.coins || [])].reduce<
    RecordOptions['coins']
  >((acc, curr) => {
  ...
  const contentHash = overwrites.contentHash || defaultRecords.contentHash
...
test.describe('profile', () => {
  test('should display profile records', async ({
    page,
    login,
    makeName,
    makePageObject,
    ...
    const name = await makeName({
      label: 'profile',
      type: 'legacy',
      records: await makeRecords({ accounts }),
    })
...
test.describe('migrations', () => {
    ...
    await generateRecords({ accounts })({
      ...
      records: await makeRecords(
        { accounts },
        {
          ...
          coins: [{ coin: 'eth', value: accounts.getAddress('user2') }],
  ...
  test('should be able migrate the current profile when the current and latest resolver are not in sync', async ({
    ...
    await expect(recordsPage.getRecordValue('contentHash')).toHaveText(
      getDefaultRecords(undefined).contentHash!,
    )
    await expect(recordsPage.getRecordValue('abi')).toHaveText('{"test":"test"}')
  })
...
test.describe('unwrapped', () => {
  test('should be able to add/update profile records without migration', async ({
    ...
    await expect(profilePage.record('text', 'description')).toHaveText('Hello2')
    await expect(profilePage.record('text', 'url')).toHaveText('twitter.com')
    await expect(profilePage.record('address', 'eth')).toHaveText(accounts.getAddress('user', 1))
    await expect(profilePage.record('address', 'btc')).toHaveText('bc1qj...pwa6n')
    await expect(profilePage.record('address', 'etcLegacy')).toHaveText('etcLegacy0x3C4...293BC')
    await expect(profilePage.record('text', 'email')).toHaveText('fakeemail@fake.com')
    ...
    // Update records
    await profilePage.profileEditorInput('eth').fill(accounts.getAddress('user2'))
