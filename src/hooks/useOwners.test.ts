...
import { makeMockWrapperData } from '@root/test/mock/makeMockUseWrapperDataData.ts'
...
const TEST_MATRIX = {
  ...
  'eth-grace-period-unwrapped-2ld': {
    ownerData: { registrant: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266' as Address },
  },
}
...
describe('useOwners', () => {
  it.each(DATA)(`should return the correct data for a %s`, (nameType, overrides) => {
    const { result } = renderHook(() =>
      useOwners({
        ownerData: makeMockUseOwnerData(nameType, overrides?.ownerData),
        wrapperData: makeMockWrapperData(nameType),
