...
const getResolverInterfaceErrors = <TInterfaceName extends string, const TInterfaceNames extends readonly TInterfaceName[]>({ interfaceNames, hasInterfaces }: { interfaceNames: TInterfaceNames; hasInterfaces: boolean[] | undefined }) => {
  if (!hasInterfaces) return []
  return interfaceNames.reduce((prev, curr, i) => {
    const hasInterface = hasInterfaces[i]
    if (!hasInterface)
      return [...prev, `Address does not support ${curr} interface`] as const
    return prev
...
export const useResolverHasInterfaces = <const TInterfaceNames extends readonly ResolverInterfaceName[], TInterfaceIds extends GetInterfaceIds<TInterfaceNames>>({
  ...
  const interfaceIds = useMemo(() => interfaceNames.map((name) => RESOLVER_INTERFACE_IDS[name]) as TInterfaceIds, [interfaceNames])

  const knownResolverData = useMemo(() => getKnownResolverData({ chainId, resolverAddress }), [chainId, resolverAddress])
  ...
  const data = useMemo(() => {
    ...
    return interfaceIds.map((interfaceId) => knownResolverData.supportedInterfaces.includes(interfaceId)) as GetSupportedInterfacesReturnType<TInterfaceIds>
  }, [knownResolverData, data_, interfaceIds])

  const errors = getResolverInterfaceErrors<ArrayToUnion<TInterfaceNames>, TInterfaceNames>({ interfaceNames, hasInterfaces: data })
