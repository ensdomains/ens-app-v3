...
vi.mock('@app/utils/records', () => ({
  profileRecordsToKeyValue: vi.fn(async (profile) => ({ texts: profile.texts })),
...
describe('migrateProfileWithReset transaction', () => {
  ...
  it('lets a contenthash-only profile through the records floor', async () => {
    mockGetSubgraphRecords.mockResolvedValue(null)
    mockGetRecords.mockResolvedValue({
      texts: [],
      coins: [],
    ...
    expect(mockSetRecords).toHaveBeenCalledTimes(1)
  })
})
