...
const mockCheckCanSend = jest.fn()
jest.mock('@app/transaction-flow/input/SendName/utils/checkCanSend', () => ({
  checkCanSend: () => mockCheckCanSend()
}))

const mockCheckCanSyncManager = jest.fn()
jest.mock('@app/transaction-flow/input/SyncManager/utils/checkCanSyncManager', () => ({
  checkCanSyncManager: () => mockCheckCanSyncManager()
}))

const mockGetAvailableRoles = jest.fn().mockReturnValue([])
jest.mock('@app/hooks/ownership/useRoles/utils/getAvailableRoles', () => ({
...
describe('useRoleActions', () => {
  it('should include the ability to send eth name if checkCanSend return true', async () => {
    ...
    expect(result.current.data).toEqual(expect.arrayContaining([expect.objectContaining({type: 'send-name'})]))
    expect(result.current.data).not.toEqual(expect.arrayContaining([expect.objectContaining({type: 'send-dns'})]))
  })
  ...
  it('should include the ability to send dns name if checkCanSend returns true', async () => {
    ...
    expect(result.current.data).toEqual(expect.arrayContaining([expect.objectContaining({type: 'send-dns'})]))
    expect(result.current.data).not.toEqual(expect.arrayContaining([expect.objectContaining({type: 'send-name'})]))
  })
  ...
  it('should include the ability to refresh dns name if user is logged in and name is not .eth', async () => {
    ...
    expect(result.current.data).toEqual(expect.arrayContaining([expect.objectContaining({type: 'refresh-dns'})]))
  })
  ...
  it('should not include the ability to refresh dns name if user is logged in and name is .eth', async () => {
    ...
    expect(result.current.data).not.toEqual(expect.arrayContaining([expect.objectContaining({type: 'refresh-dns'})]))
  })
  ...
  it('should include the ability to sync manager if checkCanSyncManager returns true', async () => {
    ...
    expect(result.current.data).toEqual(expect.arrayContaining([expect.objectContaining({type: 'sync-manager'})]))
  })
  ...
  it('should not include the ability to sync manager checkCanSyncManager returns false', async () => {
    ...
    expect(result.current.data).not.toEqual(expect.arrayContaining([expect.objectContaining({type: 'sync-manager'})]))
  })
  ...
  it('should include the ability to edit roles if getAvailableRoles returns at least one result', async () => {
    ...
    expect(result.current.data).toEqual(expect.arrayContaining([expect.objectContaining({type: 'edit-roles'})]))
  })
  ...
  it('should not include the ability to edit roles if getAvailableRoles returns at zero result', async () => {
    ...
    expect(result.current.data).not.toEqual(expect.arrayContaining([expect.objectContaining({type: 'edit-roles'})]))
  })
})