...
const Profile = ({ name, resolverExists }: Props) => {
  const { t } = useTranslation('register')

  const existingRegistrationData = useTransactionManager((s) =>
    s.getCurrentRegistrationFlowOrDefault(name),
  )
  const onRegistrationProfileStepCompleted = useTransactionManager(
    (s) => s.onRegistrationProfileStepCompleted,
  )

  const defaultResolverAddress = useContractAddress({ contract: 'ensPublicResolver' })
  const clearRecords =
    existingRegistrationData.resolverAddress === defaultResolverAddress ? resolverExists : false
  ...
  const {
    ...
    hasErrors,
  } = useProfileEditorForm(existingRegistrationData.records)
  ...
  const onSubmit = (data: ProfileEditorForm, e?: BaseSyntheticEvent<object, any, any>) => {
    ...
    onRegistrationProfileStepCompleted(name, {
      records: newRecords,
      clearRecords,
      resolverAddress: existingRegistrationData.resolverAddress,
          ...
          match(field)
            ...
            .otherwise(() => (
              <ProfileRecordInput
                ...
                recordKey={field.key}
                group={field.group}
                disabled={field.key === 'eth' && existingRegistrationData.reverseRecord}
                label={labelForRecord(field)}
                secondaryLabel={secondaryLabelForRecord(field)}
                placeholder={placeholderForRecord(field)}
                error={errorForRecordAtIndex(index)}
