...
const Registration = ({ nameDetails, isLoading }: Props) => {
  ...
  const currentRegistrationFlowStep = useTransactionManager((s) =>
    s.getCurrentRegistrationFlowStep(normalisedName),
  )
  const clearRegistrationFlow = useTransactionManager((s) => s.clearRegistrationFlow)
  const moonpayUrl = useTransactionManager(
    (s) => s.getCurrentRegistrationFlowOrDefault(normalisedName).externalTransactionData?.url,
  )
  ...
  } = useMoonpayRegistration(normalisedName)
  ...
  useEffect(() => {
    const handleRouteChange = (e: string) => {
      if (e !== router.asPath && currentRegistrationFlowStep === 'complete') {
        clearRegistrationFlow(normalisedName)
      }
    }
  ...
  return (
    <>
      ...
      <Content
        ...
        hideHeading={currentRegistrationFlowStep === 'complete'}
      ...
      >
        {{
          ...
          trailing: match([labelTooLong, currentRegistrationFlowStep])
            ...
            .with([false, 'profile'], () => (
              <Profile name={normalisedName} resolverExists={resolverExists} />
            ))
            .with([false, 'info'], () => <Info name={normalisedName} />)
            .with([false, 'transactions'], () => <Transactions name={normalisedName} />)
            .with([false, 'complete'], () => (
              <Complete name={normalisedName} beautifiedName={beautifiedName} />
            ))
            .exhaustive(),
        }}
      </Content>
