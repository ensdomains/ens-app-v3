...
const raw = async ({ contracts, multicallWrapper }, name) => {
    ...
    const isOldDNSRegistrarTx = {
        ...
        data: baseRegistrar?.interface.encodeFunctionData('supportsInterface', [
            consts_1.interfaces.DNSSEC_CLAIM_OLD,
        ]),
    };
    const isNewDNSRegistrarTx = {
        to: baseRegistrar?.address,
        data: baseRegistrar?.interface.encodeFunctionData('supportsInterface', [
            consts_1.interfaces.DNSSEC_CLAIM_NEW,
        ]),
    };
    const tldOwnerTx = {
        to: baseRegistrar?.address,
        data: baseRegistrar?.interface.encodeFunctionData('ownerOf', [
            (0, labels_1.labelhash)(tld),
        ]),
    };
...
const decode = async ({ contracts, multicallWrapper }, data, name) => {
    ...
    const baseRegistrar = await contracts?.getBaseRegistrar();
    try {
        const [isOldDNSRegistrar] = baseRegistrar.interface.decodeFunctionResult('supportsInterface', result[0].returnData);
        const [isNewDNSRegistrar] = baseRegistrar.interface.decodeFunctionResult('supportsInterface', result[1].returnData);
        const [tldOwner] = baseRegistrar.interface.decodeFunctionResult('ownerOf', result[1].returnData);
        return {
            isOldDNSRegistrar,
            isNewDNSRegistrar,
            tldOwner,
