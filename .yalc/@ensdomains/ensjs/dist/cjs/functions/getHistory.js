...
exports.getHistoryDetailForTransactionHash =
  exports.getHistoryWithDetail =
  exports.getHistory =
    void 0
...
const eventFormat = {
  ...
  Resolver: {
    ...
    MulticoinAddrChanged: (args) => {
      const format =
        address_encoder_1.formatsByCoinType[parseInt(args.coinType)]
      if (!format) {
        return {
    ...
    ContenthashChanged: (args) => ({
      hash: (0, contentHash_1.decodeContenthash)(args.hash),
    }),
...
const mapEvents = (eventArray, type) =>
...
const mapResultDetailDecode = (publicResolver) => (result) => {
  const hashIndexes = (0, makeHashIndexes_1.makeOtherIndexes)(
    result.input,
    '10f13a8c',
  )
  const abiLengths = hashIndexes.map((x) => ({
    ...
    length:
      x === 0
        ...
        : parseInt(
            ethers_1.ethers.utils.hexDataSlice(result.input, x - 32, x),
            16,
          ),
  }))
  const ABIs = abiLengths.map(({ index, length }) =>
    ethers_1.ethers.utils.hexDataSlice(result.input, index, index + length),
  )
  return ABIs.map((abi) => {
    try {
...
async function getHistoryWithDetail(
  ...
  const fetchResult = (
    await ethers_1.ethers.utils.fetchJson(
      provider.connection,
      JSON.stringify(
...
async function getHistoryDetailForTransactionHash(
  { contracts, provider },
  hash,
  indexInTransaction,
) {
