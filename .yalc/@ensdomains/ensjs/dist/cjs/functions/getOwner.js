...
var import_abi_coder = require("@ethersproject/abi/lib/abi-coder");
...
`;
const singleContractOwnerDecode = (data) => import_abi_coder.defaultAbiCoder.decode(["address"], data)[0];
const decode = async ({
  contracts,
  multicallWrapper,
  gqlInstance
  ...
  const decodedData = [result[0][1], result[1][1], (_a = result[2]) == null ? void 0 : _a[1]].map(
    (ret) => ret && ret !== "0x" && import_abi_coder.defaultAbiCoder.decode(["address"], ret)
  );
