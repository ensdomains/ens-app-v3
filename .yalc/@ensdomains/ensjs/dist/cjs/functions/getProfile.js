...
const makeHashIndexes = (data, name) =>
  [...data.matchAll(ethers_1.ethers.utils.namehash(name).substring(2))].map(
    (x) => x.index / 2 - 1,
  )
const getDataForName = async (
  { contracts, _getAddr, _getContentHash, _getText, resolverMulticallWrapper },
  name,
  options,
) => {
  ...
  let resolver
  try {
    resolver = await universalResolver?.resolve(
...
const formatRecords = async (
  { _getText, _getAddr, _getContentHash },
  data,
  calls,
  options,
) => {
    ...
    await Promise.all(
      data.map(async (item, i) => {
        ...
        if (itemRet.type === 'addr' || itemRet.type === 'contenthash') {
          decodedFromAbi = ethers_1.ethers.utils.defaultAbiCoder.decode(
            ['bytes'],
            item,
          )[0]
  ...
  if (
    ...
    if (
      ...
      ethers_1.ethers.utils.isBytesLike(options.contentHash.decoded) &&
...
const graphFetch = async ({ gqlInstance }, name, wantedRecords) => {
  ...
  Object.keys(wantedRecords).forEach((key) => {
    ...
    if (typeof data === 'boolean' && data) {
      if (key === 'contentHash') {
        returnedRecords[key] = (0, contentHash_1.decodeContenthash)(
          resolverResponse.contentHash,
        )
      } else {
